...
def smart_plus_2d(p1,p2):
    """
    See guidance.pdf
    p1, p2 are two poses (x1, z1, yaw1) and (x2, z2, yaw2)
    """
    R = get_so2(p1[2])
    t = p1[:2] + (R @ p2[:2])
...
def smart_minus_2d(p2, p1):
    """
    See guidance.pdf
    p2, p1 (note the order) are two poses (x2, z2, yaw2) and (x1, z1, yaw1)
    """
    R = get_so2(p1[2])
    t = R.T @ (p2[:2]-p1[:2])
...
def clean_point_cloud(point_cloud):
    """
    Clean raw KITTI LiDAR point cloud
    """
    intensity_threshold = 0.05
    valid_intensity = point_cloud[:, 3] > intensity_threshold
    valid_height1 = (point_cloud[:, 2] > -1.0)
    valid_height2 = (point_cloud[:, 2] < 1.5)
    valid_distance = (np.linalg.norm(point_cloud[:, :2], axis=1) < 30)
    valid_points = valid_intensity & valid_height1 & valid_height2 & valid_distance
